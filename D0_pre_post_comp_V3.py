...
# comparision of pre and post outputs

compop = wb_op.get_sheet_by_name('output')
for x in range(len(cell_list)):
    for y in range(len(postcell_list)):
        if postcell_list[y] == cell_list[x]:
            compop.cell(row=x+2,column=1).value = cell_list[x]
            compop.cell(row=x+2,column=2).value = pre_rrc_sum[x]
            compop.cell(row=x+2,column=3).value = post_rrc_sum[y]
            compop.cell(row=x+2,column=4).value = pre_thpt_sum[x]
            compop.cell(row=x+2,column=5).value = post_thpt_sum[y]
        else:
            continue
wb_op.save('output.xlsx')

for x in range(2,compop.max_row+1):
    if compop.cell(row=x,column=2).value != 0:
        compop.cell(row=x,column=6).value = (((compop.cell(row=x,column=3).value)-(compop.cell(row=x,column=2).value))/(compop.cell(row=x,column=2).value))*100
    else:
        compop.cell(row=x,column=6).value = 'No Pre RRC traffic'
        
for x in range(2,compop.max_row+1):
    if compop.cell(row=x,column=4).value != 0 and compop.cell(row=x,column=4).value != 'N.A' and compop.cell(row=x,column=5).value != 0 and compop.cell(row=x,column=5).value != 'N.A':
        compop.cell(row=x,column=7).value = (((compop.cell(row=x,column=5).value)-(compop.cell(row=x,column=4).value))/(compop.cell(row=x,column=4).value))*100
    else:
        compop.cell(row=x,column=7).value = 'No Pre DL Throughput'       
