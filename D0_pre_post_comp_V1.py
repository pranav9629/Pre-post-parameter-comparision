...
for x in range(2,len(cell_list)+1):
    preop.cell(row=x,column=1).value = cell_list[x-2]
    preop.cell(row=x,column=2).value = pre_rrc_sum[x-2]
    preop.cell(row=x,column=3).value = pre_thpt_sum[x-2]
for x in range(2,len(postcell_list)+1):
    postop.cell(row=x,column=1).value = postcell_list[x-2]
    postop.cell(row=x,column=2).value = post_rrc_sum[x-2]
    postop.cell(row=x,column=3).value = post_thpt_sum[x-2]
